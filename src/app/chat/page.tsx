...
export default function ChatPage() {
  const { chats, activeChatId, isStreaming, setChats, setActiveChat, addChat, addMessage, updateLastMessage, setStreaming, updateChatTitle } = useChatStore();
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-hide">
            {chats.map(c => (
              <button key={c.id} onClick={() => setActiveChat(c.id)}
                className={cn("w-full text-left px-3 py-2.5 rounded-xl text-sm truncate transition-colors",
